...
function initFloatingElements() {
  const floatingElements = document.querySelectorAll('.floating-guitar, .floating-scooby, .floating-max');
  ...
  function updateCountdown() {
    ...
    if (distance > 0) {
      ...
      if (!countdownElement) {
        ...
        countdownElement.className = 'countdown-timer flex justify-center gap-8 my-8 flex-wrap';
        ...
        if (heroTitle) {
          heroTitle.parentNode.insertBefore(countdownElement, heroTitle.nextSibling);
        }
      } else {
...
function initParallax() {
  window.addEventListener('scroll', function() {
    ...
    const parallaxElements = document.querySelectorAll('.floating-guitar, .floating-scooby, .floating-max');
...
function initInteractiveElements() {
  ...
  document.querySelectorAll('.flex.items-start.p-6.bg-white.rounded-2xl.shadow-lg, .bg-white.rounded-2xl.overflow-hidden.shadow-lg, .meeting-card').forEach(item => {
...
document.addEventListener('keydown', function(e) {
  ...
  if (konamiCode.join(',') === konamiSequence.join(',')) {
    ...
    showNotification('🎉 Контра-код активирован! Мальчишник будет еще круче! 🎉', 'success');
    createConfetti();
